...
logger = logging.getLogger(__name__)
    ...
    # The comparator_lookup maps the STIX pattern operators to the data source query operators.
class CloudIdentityQueryStringPatternTranslator:
    ...
    def _format_set(values) -> str:
        ...
        return "({})".format(' OR '.join([CloudIdentityQueryStringPatternTranslator._escape_value(value) for value in gen]))

    @staticmethod
    def _format_match(value) -> str:
        raw = CloudIdentityQueryStringPatternTranslator._escape_value(value)
        if raw[0] == "^":
            raw = raw[1:]
        else:
            raw = ".*" + raw
    ...
    def _format_like(value) -> str:
        ...
        return CloudIdentityQueryStringPatternTranslator._escape_value(value)
